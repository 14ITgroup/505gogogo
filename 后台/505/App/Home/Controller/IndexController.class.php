		...
		if (IS_POST) {
			...
			if ($useraccount == "" || $password == "") {
				...
				if ($result) {
					...
					redirect(U("index.php/Home/Index/homepage"), 1, "登录成功");
				} else {
					//$this->error('登录失败');
					redirect(U("index.php/Home/Index/login"), 1, "登录失败");
				}
			}
		}
	}
	...
	public function register() {
		...
		if (IS_POST) {
			...
			if ($result) {
				...
				redirect(U("index.php/home/Index/login"), 1, "注册成功");
			} else {
				//$this->error('注册失败');
				redirect(U("index.php/home/Index/login"), 1, "注册失败");
			}
		}
	}
...
//如果是直接加载首页的话
	...
	public function pay() {
		if (isset($_SESSION['useraccount'])) {
			...
			if (isset($_GET['name'])) {
//&&isset($_GET['color'])&&isset($_GET['size'])
				$goodsname = $_GET['name'];
...
//判断用户登录
			...
			for ($i = 0; $i < $length; $i++) {
				...
				if ($goodsleft - $uniquecount[$i] > 0) {
//如果库存足够，执行库存减去goodsnum的操作和生成订单的操作
